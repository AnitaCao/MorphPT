...
class CellParquetBase(Dataset):
    ...
    def __init__(
        ...
        # Load: single file or directory of shards
        ...
        cols_needed = [label_col, tissue_col, x_col, y_col, cell_id_col]
        missing = [c for c in cols_needed if c not in df.columns]
        if missing:
            raise RuntimeError(f"Missing columns {missing} in {self.parquet_path}. Got {list(df.columns)}")
