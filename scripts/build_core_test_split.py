...
=======================================================================
1. 从 test_shards 中按细类(Fine Class)在所有切片(per-slide)中做均匀抽样，生成核心 Benchmark。
...
def stratified_sample(group, n_target, rng):
    """Sample n_target from group, stratified evenly across slides.
    Uses a water-filling algorithm to distribute exactly n_target cells 
    as evenly as possible across all available slides."""
    ...
    allocs = np.zeros(len(slide_counts), dtype=int)
    remaining = n_target
    
    while remaining > 0:
        headroom = slide_counts - allocs
        active = np.sum(headroom > 0)
        if active == 0:
            break
            
        add = max(1, remaining // active)
        # Randomize order to prevent bias when distributing remainders
        indices = rng.permutation(len(slide_counts))
        for i in indices:
            if headroom[i] > 0:
                give = min(headroom[i], add)
...
def main():
    ...
    ap.add_argument("--blacklist_path", help="Path to hard_failure_ids_v2.csv")
    ap.add_argument("--out_dir", required=True)
    ap.add_argument("--cells_per_class", type=int, default=10000)
    ...
    test_files = [f for f in test_files if "bad_ids" not in f] 
    if not test_files:
        raise RuntimeError(f"No parquet files in {args.test_shards_dir}")

    print(f"Loading {len(test_files)} test shards from {args.test_shards_dir}")
    ...
    if args.blacklist_path:
        blacklist_df = pd.read_csv(args.blacklist_path)
        blacklist_ids = set(blacklist_df["cell_id"].unique())
        print(f"Loaded {len(blacklist_ids):,} blacklisted cell IDs from {args.blacklist_path}")
        
        # Filter out blacklisted cells
        df_all = df_all[~df_all["cell_id"].isin(blacklist_ids)]
        print(f"Total cells after blacklist removal: {len(df_all):,}")
