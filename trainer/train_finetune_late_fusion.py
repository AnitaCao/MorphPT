...
class DinoV3Classifier(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if x.ndim == 5:
            ...
            z_list = []
            for s in range(S):
                z_list.append(self.head(Pn[:, s]))          # each view → logits
            aux["z_list"] = z_list

            # Fuse logits (not embeddings)
            if self.fuse == "avg":
                fused_logits = sum(z_list) / S
                aux["g"] = torch.full((B, S), 1.0 / S, device=x.device, dtype=Pn.dtype)
            elif self.fuse == "gate":
                fused_emb, g = self.gate(Pn)                # gate-weighted embedding
                fused_emb = F.normalize(fused_emb, dim=-1)
                fused_logits = sum(g[:, s:s+1] * z_list[s] for s in range(S))
                aux["g"] = g
                aux["emb_fused_gate"] = fused_emb           # for extraction comparison
            else:
                raise ValueError(f"Unknown fuse={self.fuse}")

            # Fused embeddings for downstream
            emb_avg = F.normalize(Pn.mean(dim=1), dim=-1)
            aux["emb_fused_avg"] = emb_avg
            emb = aux.get("emb_fused_gate", emb_avg)       # prefer gate if available

            return fused_logits, emb, aux
...
def train_one_epoch(model, loader, opt, scaler, device, amp_dtype, num_classes, cons_w=0.0, epoch=1):
    ...
    T_cons = 3.0  # temperature for KL consistency
    ...
    for x, y, _tissue in loader:
        ...
        with torch.amp.autocast("cuda", dtype=amp_dtype, enabled=torch.cuda.is_available()):
            ...
            # ── KL consistency between per-view logits ──
            loss_cons = torch.tensor(0.0, device=device)
            if effective_cons_w > 0 and "z_list" in aux:
                z0, z1 = aux["z_list"]
                p0 = F.log_softmax(z0 / T_cons, dim=1)
                q0 = F.softmax(z1 / T_cons, dim=1)
                p1 = F.log_softmax(z1 / T_cons, dim=1)
                q1 = F.softmax(z0 / T_cons, dim=1)
                loss_cons = (F.kl_div(p0, q0, reduction="batchmean")
                           + F.kl_div(p1, q1, reduction="batchmean")) * (T_cons * T_cons)
...
def eval_epoch(model, loader, device, amp_dtype, num_classes):
    ...
    # Logits KL consistency accumulator
    kl_sum = 0.0
    kl_n = 0
    T_cons = 3.0
    ...
    with torch.amp.autocast("cuda", dtype=amp_dtype, enabled=torch.cuda.is_available()):
        for x, y, _tissue in loader:
            ...
            if "z_list" in aux:
                ...
                p1 = F.log_softmax(z1 / T_cons, dim=1)
                q1 = F.softmax(z0 / T_cons, dim=1)
                kl = (F.kl_div(p0, q0, reduction="batchmean")
                    + F.kl_div(p1, q1, reduction="batchmean")) * (T_cons * T_cons)
                kl_sum += kl.item() * y.size(0)
                kl_n += y.size(0)
    ...
    val_kl = kl_sum / max(1, kl_n)
    return loss_sum / max(1, n), acc, mf1, cv_cos, val_kl
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        va_loss, va_acc, va_f1, va_cv_cos, va_kl = eval_epoch(model, val_loader, device, amp_dtype, num_classes)
        dt = time.time() - t0
        ...
        print(f"epoch {epoch:02d}  "
              ...
              f"cv_cos={va_cv_cos:.4f}  val_kl={va_kl:.4f}  lr={cur_lr:.2e}  secs={dt:.1f}")
